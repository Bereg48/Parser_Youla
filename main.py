...
class BOT:
        ...
        if emptyDB == False:
            ...
            for i in range(len(listCity)):
                ...
                sql = "CREATE TABLE " + str(
                    simbol) + " (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, city VARCHAR(100) NOT NULL, url VARCHAR (1000) NOT NULL)"
                ...
                for i in range(len(subCity)):
                    ...
                    sql = "INSERT INTO " + str(simbol) + " (city, url) VALUES (\'" + subCity[
                        i].getText() + "\'" + ",\'" + subCityHref + "\')"
                    cursor.execute(sql)
                    conn.commit()
                    print(
                        '\r' + 'Заполнено {bow} данных из {total} [ {proc} % ]'.format(bow=side, total=len(subCity) - 1,
                                                                                       proc=round(
                                                                                           (side * 100) / len(subCity),
                                                                                           1)), end='')
                    side += 1
                system('cls')
        else:
    ...
    def pars_page(self, url, numbers):
        ...
        btn = self.br.find_element_by_xpath(
            '//*[@id="app"]/div[1]/div[3]/section/div/div/div/div[1]/div[2]/ul/li[1]/div/div[2]/button')
        btn.click()
        ...
        name = self.br.find_element_by_xpath(
            '//*[@id="app"]/div[1]/div[3]/section/div/div/div/div[1]/div[4]/ul/li[1]/a/div/div[1]/div/div[2]/p')
        # print(name.text)
        # name = name.replace('&nbsp;', ' ')
